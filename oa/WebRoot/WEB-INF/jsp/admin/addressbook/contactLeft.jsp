...
<title>OA办公管理系统-模块管理</title>
	...
	<script type="text/javascript">
		$(function(){
			...
			//添加根节点
			d.add(-2, -1, "联系人管理");
			d.add(0, -2, "全部","${path}/admin/addressbook/selectContact","全部联系人","rightFrame");
			//异步请求根据数据库内容加载节点
			//返回数据格式: [[id,name],[],...]
			$.getJSON("${path}/admin/addressbook/loadContactTree","post",function(data,status){
				if(status=="success"){
					$.each(data,function(){
						d.add(this[0],0,this[1],"${path}/admin/addressbook/selectContact?contactGroup="+this[0],this[1],"rightFrame");
					});
				d.openAll();
